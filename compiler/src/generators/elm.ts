...
export function generateElm(ast: AST, outputDir: string): void {
  for (const speck of ast.specks) {
    ...
    fs.writeFileSync(path.join(srcDir, 'Main.elm'), emitMain(speck, stateVars, actions, recordTypes));
...
function emitMain(speck: SpeckNode, stateVars: any[], actions: ActionNode[], recordTypes: Map<string, any[]>): string {
  const L: string[] = [];
...
// actionRoute/actionBody cases are emitted by the loop below (already in file after httpError? no — they were after). We append them here via the actions loop:
  ...
  L.push(`    , body =`);
  L.push(`        [ div [ attribute "data-app" "${snakeCase(speck.name)}" ]`);
  ...
  L.push(`        Just e ->`);
  L.push(`            div [ style "color" "red" ] [ text ("error: " ++ e) ]`);
  L.push(``);
  L.push(`        Nothing ->`);
  L.push(`            div [] []`);
  L.push(``);
