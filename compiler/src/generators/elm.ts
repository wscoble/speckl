...
function emitCompositeMain(
  ...
  L.push(`                , span [ class "gb-title" ] [ text "Greybeard" ]`);
  L.push(`                ]`);
  ...
  L.push(``);
  L.push(`{-| Kanban transition pattern. THE STATE MACHINE, AS DATA:`);
  L.push(``);
  L.push(`    legalTargets spec r`);
  L.push(``);
  L.push(`is the single query — the legal transitions for a card from where it`);
  L.push(`sits, compiled from the spec guards (machine.check) and the spec's own`);
  L.push(`column vocabulary. EVERY affordance reads this one function: row`);
  L.push(`highlighting while dragging, menu move items, and the commit path`);
  L.push(`(which re-checks before dispatching). The server remains authoritative.`);
  L.push(``);
  L.push(`The gesture is pointer-drag; the guard-filtered menu is the WCAG 2.5.7`);
  L.push(`keyboard/screen-reader path. Both dispatch the same bubbled message. -}`);
  L.push(`type alias KanbanSpec rec =`);
  L.push(`    { idOf : rec -> Int`);
  L.push(`    , columnOf : rec -> String`);
  L.push(`    , order : List String`);
  L.push(`    , canMove : rec -> String -> Bool`);
  L.push(`    , moveMsg : rec -> String -> FrontMsg`);
  ...
  L.push(``);
  L.push(``);
  L.push(`{-| The one transition query. -}`);
  L.push(`legalTargets : KanbanSpec rec -> rec -> List String`);
  L.push(`legalTargets spec r =`);
  L.push(`    List.filter (spec.canMove r) spec.order`);
  L.push(``);
  L.push(``);
  ...
  L.push(``);
  L.push(`        -- the card being carried: every row highlights against ITS transitions`);
  L.push(`        draggedRec =`);
  L.push(`            case page.drag of`);
  L.push(`                Just d ->`);
  L.push(`                    Dict.get (String.fromInt d.id) dict`);
  L.push(``);
  L.push(`                Nothing ->`);
  L.push(`                    Nothing`);
  L.push(``);
  L.push(`        dragFromGate =`);
  L.push(`            case ( page.drag, draggedRec ) of`);
  L.push(`                ( Just _, Just r ) ->`);
  ...
  L.push(`        (List.map (kanbanRow page spec dragFromGate draggedRec) (known ++ unknown) ++ [ ghost ])`);
  L.push(``);
  L.push(``);
  L.push(`{-| One status row. Highlight tiers, consistent by construction:`);
  L.push(`  1. gate semantic (approve green / deny red) — when dragging from the`);
  L.push(`     gate column and this row is a gate target`);
  L.push(`  2. neutral dashed — any other legal target`);
  L.push(`  3. hover ring — on top of 1 or 2, never replacing them`);
  L.push(`Illegal rows get nothing. -}`);
  L.push(`kanbanRow : Page -> KanbanSpec rec -> Bool -> Maybe rec -> ( String, List rec ) -> Html FrontMsg`);
  L.push(`kanbanRow page spec dragFromGate draggedRec ( col, items ) =`);
  L.push(`    let`);
  L.push(`        legal =`);
  L.push(`            case draggedRec of`);
  L.push(`                Just r -> List.member col (legalTargets spec r)`);
  L.push(``);
  ...
  L.push(`        isGateTarget =`);
  L.push(`            case spec.gate of`);
  L.push(`                Just g -> List.member col g.targets`);
  L.push(``);
  L.push(`                Nothing -> False`);
  L.push(``);
  L.push(`        rowClass =`);
  L.push(`            String.join " "`);
  L.push(`                ([ "gb-board-row" ]`);
  L.push(`                    ++ (if legal && dragFromGate && isGateTarget then`);
  L.push(`                            if List.member col approveTargets then`);
  ...
  L.push(`                            []`);
  L.push(`                    )`);
  L.push(`                    -- hover ring is additive: never replaces the semantic color`);
  ...
  L.push(`                        [ on "mouseup" (D.succeed (DragDrop (spec.moveMsg r col))) ]`);
  L.push(``);
  L.push(`                    else`);
  L.push(`                        []`);
  L.push(``);
  ...
  for (const [recName, entries] of affordances) {
    ...
    L.push(`                Err _ ->`);
    L.push(`                    False`);
    L.push(`    , moveMsg = \\r target -> FromMachine (${comp.module} (${an} ${ctorArgList}))`);
...
.gb-section > .gb-board { padding: var(--gb-s2) var(--gb-s3) var(--gb-s3); }
.gb-section > .gb-create-row { padding: var(--gb-s2) var(--gb-s5) var(--gb-s3); }

/* rows / cards */
.gb-row { padding: var(--gb-s2) 0; font-size: 14px; color: var(--gb-text-2); border-bottom: 1px solid rgba(205, 214, 244, 0.06); }
.gb-row:last-child { border-bottom: none; }
.gb-empty { color: var(--gb-text-2); font-size: 14px; font-style: italic; }
...
.gb-btn-approve:hover { background: #b9eba6; }
.gb-btn-deny {
  background: #f38ba8;
}
...
.gb-inline-action .gb-input { background: #1e1e2e; }
